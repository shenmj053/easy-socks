...
public class Aes256GcmCipher {
    ...
    private final static String CIPHER_AEAD_128_GCM = "aes-128-gcm";
    private final static String CIPHER_AEAD_256_GCM = "aes-256-gcm";
    ...
    public Aes256GcmCipher(String name, String password) {
        this.name = name.toLowerCase();
        if (name.equals(CIPHER_AEAD_128_GCM)) {
            this.keyLength = 16;
        } else if (name.equals(CIPHER_AEAD_256_GCM)) {
            this.keyLength = 32;
        }
...
//        }
    ...
    public byte[] decrypt(byte[] src) throws Exception {
        ...
        if (!decryptSaltSet || isForUdp) {
            ...
            srcBuf.readBytes(salt, 0, getSaltLength());
    ...
    private byte[] genSubKey(byte[] salt) {
        ...
        byte[] subKeyBytes = new byte[keyLength];
        hkdf.generateBytes(subKeyBytes, 0, keyLength);
        return subKeyBytes;
    }
...
//        Arrays.fill(rawBytes, (byte) 'd');
        byte[] rawBytes = "abcdefg".getBytes();
